...
console.log(transformName("jhoylasq castro"))*/
  ...
  useEffect(() => {
    const Data = async () => {
      ...
      if (user?.data?.user?.identities?.[0].id) {
        const proveedor = await selecSupplierEmployeeByProfileId(
          user?.data?.user?.identities?.[0].id,
        );
  ...
  const supplierData = async () => {
    ...
    const pending = selectInvoice_data({
      limit: 300,
      page: 1,
      equals: { supplier_id: idsup, state: "pending" },
    });
    const rejected = selectInvoice_data({
      limit: 300,
      page: 1,
      equals: { supplier_id: idsup, state: "rejected" },
    });
    const poruser = selectInvoice_data({
      limit: 300,
      page: 1,
      equals: { supplier_id: idsup, last_modified_by: iduser },
    });
  ...
  return (
    <ChakraProvider>
      <div className="flex h-screen w-full items-center justify-center bg-gradient-to-tr from-green-900 to-green-700 lg:w-full">
        {!isinicio ?
          <HStack spacing={6}>
            <Box className="relative rounded-3xl bg-gray-200 px-10 py-10">
              <Stack>
                <VStack>
                  <HStack justify="center" align="center">
                    <Image src="/zoft.png" alt="ZOFT" w="55px" h="55px" />
                    <VStack spacing={0}>
                  ...
                  <HStack className="rounded-2xl bg-gray-300 px-2">
                    <Image
                  ...
                  <HStack
                    width="95%"
                    className="rounded-2xl bg-yellow-300 px-2"
                    padding={5}
                    marginTop="10%"
                    marginBottom="10%"
                    align="stretch"
                    ...
                    <VStack
                      ...
                      textAlign="start"
                      align="start">
                      <Text className="font-bold" fontSize="70%">
                        OC pendientes de revision:{" "}
                      </Text>
                      <Text className="font-bold" fontSize="70%">
                        OC rechazados:{" "}
                      </Text>
              ...
              <HStack
                ...
                <Skeleton isLoaded={!Squeleton}>
                  <HStack>
                    ...
                    <Tooltip label={name}>
                      <Text fontSize="80%">
                        {name ? transformName(name) : "Invalido"}
                      </Text>
                    </Tooltip>
                  </HStack>
                </Skeleton>
            ...
            <Box className="relative rounded-3xl bg-gray-200 px-10 py-10">
              <Stack>
                <VStack>
                  <HStack
                    className="relative rounded-2xl bg-yellow-300 px-2"
                    width="270px"
                    justify="end"
                    align="end">
                    <HStack
                  ...
                  <Text fontSize="110%" className="font-bold">
                    OPCIONES DE FACTURA
                  </Text>
              ...
              <VStack
                ...
                justify="center"
                textAlign="center">
                <Heading marginTop="7%" fontSize="80%">
                  © {new Date().getFullYear()} Reficar - Todos los derechos
                  reservados.
                </Heading>
              </VStack>
            </Box>
        ...
        : <>
            <div
              className={`relative flex h-[82%] w-[80%] flex-col rounded-3xl border border-gray-300 bg-gray-100 p-4 text-center shadow-md`}>
              {Add === true && (
                <Associate_invoice
