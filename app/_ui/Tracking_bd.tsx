...
import { selectSupplierData, selectSupplierDataByInvoiceID } from '../_lib/database/supplier_data';
import { selectSingleBill } from '../_lib/database/base_bills';
import { selectSingleSupplier, selectSuppliers } from '../_lib/database/suppliers';
...
export const Tracking_bd = () => {
    ...
    type OrderBy = {
        column: "state" | "created_at" | "updated_at" | "supplier_id" | "feedback" | "invoice_id" | "last_modified_by";
        options: {
          ascending: boolean;
        };
      };
                            ...
                            for (const sup of data) {
                                try {
                                    ...
                                    worksheet.addRow({
                                        ...
                                        FOB_UNIT: parseFloat(((sup.billed_unit_price / 100) / sup.trm).toFixed(8)),
                                        FACTURA: sup.bill_number,
                                        FMM: undefined,
                                        PA: subpartida,
                                        UC: measurement,
                                        TRM: sup.trm,
                                        FOB: parseFloat(((((sup.billed_unit_price / 100) * sup.billed_quantity) / sup.trm)).toFixed(2)),
                    ...
                    invoice.map(async (invo) => {
                        try {
                            ...
                            try {
                                ...
                                if(Selectyear !== "all"){
                                    if(Selectyear !== data[0].modified_at.substring(0,4)) return null
                                }
                                if(Selectmonth !== "all"){
                                    if(Selectmonth !== data[0].modified_at.substring(5,7)) return null
                                }
                    ...
                    <Box>
                        <Flex marginTop="5px" height="14%" width="100%">
                            <HStack width="100%" spacing={5}>
                                ...
                                <HStack width="20%">
                                <Button onClick={()  => Supp_Export()} colorScheme='teal' bg="#F1D803">
                        ...
                        <HStack mt={2} borderColor="gray.300" whiteSpace="nowrap" className="rounded-2xl" justifyContent='center' alignItems="center" bg="gray.200" w="100%" h="10%">
                <HStack bgColor="white" align="center" justify="center" w="100%" h="100%">
                <HStack position="relative"  alignItems="center" justify="center" w="5%">
                {Checkboxs && (
                    <Checkbox  
                    position="absolute"
                    ml="140%"
                  ...
                  <HStack overflowX="clip" ml='3%' alignItems="center" justify="start" w="25%">
                  ...
                  <HStack mr='3%' spacing={4} alignItems="center" justify="center" w="30%">
                    <Text className="font-bold" fontSize='100%'>Proveedor</Text>
                  </HStack>
                </HStack>
              </HStack>
                            ...
                            {data.map((item) => (
                                <VStack position="relative"  w="100%" key={item.orden}>
                                    ...
                                    isChecked={isChecked(item.consecutivo)}
                                     onChange={(e) => handleCheckboxChange(item.consecutivo, e.target.checked)}
                                    position="absolute"
                                       zIndex={50}         
                                                />
                                    )}
                                    ...
                                    >
                                        <HStack className="rounded-2xl" bgColor="white" align="center" justify="center" w="100%" h="100%">
                                            <HStack ml="3%" alignItems="center" justify="start" w="5%">
                                            ...
                                            <HStack spacing={0} alignItems="center" justify="center" w="20%">
                                                <Tooltip label={
                                                    <Box>
                                                        <Text>Subtotal.F: {formatMoney(item.subtotal)}</Text>
                                                        <Text>FOB: {formatMoney(item.fob)}</Text>
                                                    </Box>
                                                } bg="gray.300" placement='top' color="black" >
                                                <HStack>
