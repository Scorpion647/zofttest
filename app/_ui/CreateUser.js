...
export const CreateLargeUser = () => {
  ...
  const AsocciateProfile = async () => {
    if (parseInt(id) === 0) {
      ...
      if (supplier) {
        const ids = await selectSuppliers({
          page: 1,
          limit: 1,
          equals: { name: Searchvalue },
        });
        const obtener = await selectProfiles({
          limit: 1,
          page: 1,
          equals: { email: Email },
        });
        console.log(obtener[0].profile_id);
        const employee = await insertSupplierEmployee({
          profile_id: obtener[0].profile_id,
          supplier_id: ids[0].supplier_id,
        });

        if (employee) {
          toast({
            title: "Asociacion se realizo correctamente",
            description: `La asociacion y creacion del proveedor se realizo con exito.`,
            status: "success",
            duration: 3000,
            isClosable: true,
          });
          ...
          toast({
            title: "Asociacion no se ha podido realizar con exito",
            description:
              "Hubo un error en la creacion o asociacion del usuario al proveedor",
            status: "error",
            position: "top",
            isClosable: true,
            duration: 10000,
        ...
        toast({
          title: "Asociacion no se ha podido realizar con exito",
          description: "Hubo un error en la creacion del proveedor",
          status: "error",
          position: "top",
          isClosable: true,
          duration: 10000,
      ...
      if (employee) {
        toast({
          title: "Asociacion se realizo correctamente",
          description: `La asociacion del usuario se realizo con exito.`,
          status: "success",
          duration: 3000,
          isClosable: true,
        });
    ...
    try {
      ...
      if (!inputValue || inputValue === "") {
        ...
        filter = {
          page: 1,
          limit: 10,
          equals: { user_role: "guest", email: inputValue },
        };
      }
  ...
  const Delete = async (email) => {
    try {
      console.log("hola1");
      const user = await selectProfiles({
        limit: 1,
        page: 1,
        equals: { email: email },
      });
      ...
      if (user) {
        ...
        console.log("hola4");
        toast({
          title: "Usuario Rechazado con exito",
          description: `El Usuario se le ha negado el registro al sistema`,
          status: "success",
          duration: 3000,
          isClosable: true,
        });
      ...
      toast({
        ...
        description: `Hubo un problema al rechazar al usuario, porfavor intentelo mas tarde`,
        status: "error",
        duration: 5000,
        isClosable: true,
      });
  ...
  return (
    <>
      <HStack className="mb-3 mt-3">
        <Input
          value={inputValue}
          onChange={(e) => setInputValue(e.target.value)}
          width={iSmallScreen ? "70%" : "30%"}
          border="1px"
      ...
      <VStack
        ...
        bgColor="gray.200"
        height="400"
        justify={filteredData.length === 0 ? "center" : ""}
        align="center">
                ...
                <Box
                  ...
                  <HStack
                    ...
                    <HStack
                      ...
                      alignItems="center"
                      justify="center"
                      w={iSmallScreen ? "40%" : "20%"}>
                      <Button bg="red">
                        <CloseIcon
                          w={3}
                          h={3}
                          color="white"
                          onClick={() => Delete(item.email)}
                        />
                      </Button>
                      <Button
                        onClick={() => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
          <div className="relative z-20 w-5/6 max-w-md rounded-3xl border border-gray-300 bg-white p-4 text-center shadow-md">
            <h2 className="mb-4 text-xl font-bold">
              Confirmacion de Autorizacion de Usuario
            </h2>
            <p className="font-semibold">
              Seleccione el Proveedor al que quiere asociar este usuario
            </p>
            ...
            {id === 0 && (
              <VStack
                ...
                <Checkbox isChecked={isChecked} onChange={handleCheckboxChange}>
                  ¿Desea agregar este dominio como dominio autorizado para este
                  proveedor?
                </Checkbox>
              </VStack>
            )}
            ...
            <Box
              ...
              <HStack
                ...
                <HStack spacing={2} alignItems="center" justify="start" w="30%">
                  ...
                  <Box
                    w="5"
                    onClick={() => (setisAccept(true), setEmail(item.email))}
                    bg="green">
      ...
      {isAccept && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
          <div className="relative z-20 w-5/6 max-w-md rounded-3xl border border-gray-300 bg-white p-4 text-center shadow-md">
            ...
            <p className="font-semibold">
              ¿Esta seguro de querer agregar al siguiente usuario?
            </p>
            <p className="font-bold">{Email}</p>

            <p className="font-bold">
              No acepte esta opcion si el dominio no es privado
            </p>
