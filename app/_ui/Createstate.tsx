...
export const CreatelargeAdmin: React.FC<CreatelargeAdminProps> = ({
  sharedState,
  updateSharedState,
}) => {
  ...
  const checkfilter = async (filter: FilterType, atfilter: FilterType) => {
    if (
      JSON.stringify(previousfilter?.orderBy) !==
        JSON.stringify(filter.orderBy) &&
      JSON.stringify(previousfilter?.equals) !==
        JSON.stringify(filter.equals) &&
      previousfilter
    ) {
      setCurrentPage(1);
      setpreviousfilter(atfilter);
      setpreviouspage(false);
      const invoice = await selectInvoice_data({
        page: currentPage + 1,
        limit: filter.limit,
    ...
    const next = await selectInvoice_data({
      page: currentPage + 1,
      limit: filter.limit,
      orderBy: filter.orderBy,
      equals: filter.equals,
    ...
    const prev = await selectInvoice_data({
      page: currentPage - 1,
      limit: filter.limit,
      orderBy: filter.orderBy,
      equals: filter.equals,
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (selectedStatus !== "all") {
        if (!inputValue || inputValue === "") {
          filter = {
            ...
            orderBy: {
              column: "updated_at",
              options: { ascending: role === "administrator" ? true : false },
            },
            equals: {
              state:
          ...
          filter = await checkfilter(filter, {
            page: 1,
            limit: 7,
            orderBy: {
              column: "updated_at",
      ...
      setFilteredData(
        Data.filter((item): item is InvoiceData => item !== null),
      );
    } catch (error) {
      ...
      {!hola && (
        <>
          {!isTable ?
            <VStack
              ...
              <Flex
                ...
                {!iSmallScreen && <HStack width="20%"></HStack>}
                              ...
                              <Tooltip
                                ...
                                <Text
                                  ...
                                  onClick={(event) => {
                                    ...
                                    toast({
                                      title:
                                        "ID de Factura se ha copiado con exito",
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
                  <div className="relative z-20 w-5/6 max-w-md rounded-3xl border border-gray-300 bg-white p-4 text-center shadow-md">
                    <h2 className="mb-4 text-xl font-bold">
                      Su Solicitud de asociacion a sido Rechazada
                    </h2>
                    ...
                    <p className="mt-2 font-bold">
                      Si desea editar la asociacion anterior haga click en
                      Editar
                    </p>
                    ...
                    <HStack mt={4} justify="center" align="center">
                      ...
                      <Button
                        ...
                        colorScheme="teal"
                        className="rounded px-4 py-2"
                        onClick={() => (setisTable(true), setRazon(false))}>
                        Editar
                      </Button>
                    </HStack>
                  </div>
          ...
          : <>
              ...
              {(Type === "Edit" || Type === "View") && (
                <div
                  className={`absolute bottom-1 flex h-[100%] w-[100%] origin-center snap-center flex-col content-center justify-center self-center rounded-3xl border border-gray-300 bg-gray-100 p-4 text-center shadow-md`}>
                  <Associate_invoice
                    setisTable={setisTable}
                    isTable={Type}
                    invoi={codigo}
