          ...
          <>
            ...
            <FormControl isRequired mt={4}>
              ...
              <Select
                bgColor="white"
                name="select1"
                placeholder="Selecciones un tipo de moneda"
                onChange={handleChange}>
      ...
      try {
        ...
        if (!headersValid) {
          ...
          toast({
            title: "Error",
            description:
              "Formato de archvio incorrecto, porfavor reviselo y vuelva a intentar",
            status: "error",
            duration: 3000,
            isClosable: true,
          });
    ...
    for (const row of data) {
      ...
      if (cancelarRef.current) {
        const userConfirmed = window.confirm(
          "¿Estás seguro que quieres cancelar el proceso?",
        );
        if (userConfirmed) {
          setCancelar(false);
          toast({
      ...
      if (selectedTable === "Registros") {
        ...
        if (parseFloat(net_price) <= 0) {
          invalidbills.push({
            purchase_order,
            item: position,
            reason: "Precio neto debe ser mayor a 0",
          });
          CountGlobal.current = CountGlobal.current + 1;
          completedTasks += 1;
          if (
        ...
        if (parseFloat(unit_price) <= 0) {
          invalidbills.push({
            purchase_order,
            item: position,
            reason: "Precio unitario debe ser mayor a 0",
          });
          CountGlobal.current = CountGlobal.current + 1;
          completedTasks += 1;
          if (
        ...
        if (parseFloat(quantity) <= 0) {
          invalidbills.push({
            purchase_order,
            item: position,
            reason: "Cantidad debe ser mayor a 0",
          });
          CountGlobal.current = CountGlobal.current + 1;
          completedTasks += 1;
          if (
        ...
        if (
          !Number.isInteger(parseFloat(position)) ||
          parseInt(position) <= 0
        ) {
          invalidbills.push({
            purchase_order,
            item: position,
            reason: "Posición debe ser un entero mayor a 0",
          });
          CountGlobal.current = CountGlobal.current + 1;
          completedTasks += 1;
          if (
...
if(searchrecord.length > 0){
  ...
  continue
        ...
        if (existingRecordsMap.has(recordKey)) {
          invalidbills.push({
            purchase_order,
            item: position,
            reason: "Ya existe este OC con este item en base de datos",
          });
          CountGlobal.current = CountGlobal.current + 1;
          completedTasks += 1;
          if (
  ...
  if (!domainExists[0] || domainExists[0].name !== supplier_name) {
    ...
    supplierId = domainExists[0].supplier_id;
          ...
          const unitPriceParsed = Math.round(
            parseFloat(normalizeNumber(String(unit_price))) * 100,
          );
          ...
          if (!isNaN(unitPriceParsed)) {
            ...
            invalidbills.push({
              purchase_order,
              item: position,
              reason: "Error al procesar precios",
            });
          }
        } else {
          ...
          duplicateRecords.push({
            ...
            existing:
              recordsToInsertMap.get(recordKey) ||
              existingRecordsMap.get(recordKey),
          });
        }
        CountGlobal.current = CountGlobal.current + 1;
        completedTasks += 1;
        ...
        const normalized = String(material_code || "")
          .trim()
          .toLowerCase();
        ...
        if (!normalized || !material_code) {
          invalidmaterials.push({
            material_code: "VACIO",
            subheading: safe(subheading),
            type: safe(type),
            measurement_unit: safe(measurement_unit),
    ...
    await Promise.all([insertSupplier(suppliersToInsert)]);
  ...
  const Row = ({ index, style }) => {
    const [material_code, subheading, type, measurement_unit, reason] =
      groupedBillsArray1[index];
      ...
      if (selectedTable === "Registros") {
        ...
        if (Datafilter) {
          filter = {
            limit: 200,
            page: 1,
            equals: { purchase_order: Datafilter },
            orderBy: { column: "item" },
          };
        } else {
        ...
        if (records) {
          ...
          const suppliers = await Promise.all(
            supplierIds.map((id) => selectSingleSupplier(id)),
          );
        ...
        if (Datafilter) {
          filter = {
            limit: 50,
            page: 1,
            equals: { material_code: Datafilter },
          };
        } else {
      ...
      <Box>
          ...
          {!iSmallScreen && (
            <>
              ...
              <Switch
                ...
                isDisabled={!workbook || isProcessing}></Switch>
            </>
          )}
        </HStack>
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300">
            <div className="relative z-20 w-5/6 max-w-md rounded-3xl border border-gray-300 bg-white p-4 text-center shadow-md">
              ...
              {groupedBillsArray1.length > 0 &&
                groupedBillsArray1[0][3] === "Bills" && (
                  <>
                    <p className="mb-2 font-bold">Registros</p>
                    <Box
                      maxH="230px"
                      overflowY="auto"
                      p="2"
                      ...
                      <VStack spacing={3} align="stretch">
                        {groupedBillsArray1.map(
                          ([purchase_order, item, reason], index) => (
                            <>
                              {purchase_order && (
                                <Box
                                  key={index}
                                  ...
                                  <HStack spacing={5}>
                                    <p>
                                      <strong>OC:</strong> {purchase_order}
                                    </p>
                                    <p>
              ...
              {groupedBillsArray1.length > 0 &&
                groupedBillsArray1[0][5] === "Material" && (
                  <>
                    <p className="mb-2 font-bold">Materiales</p>
                    <Box
                      maxH="230px"
                      display="flex"
                      p="2"
              ...
              {groupedBillsArray1.length > 0 &&
                groupedBillsArray1[0][2] === "Suppliers" && (
                  <>
                    <p className="mb-2 font-bold">Proveedores</p>
                    <Box
                      maxH="230px"
                      overflowY="auto"
                      p="2"
              ...
              <p className="mt-4 font-bold">
                Registros fallidos: {groupedBillsArray1.length}
              </p>
        ...
        <HStack
          ...
          <Input
            ...
            onBlur={(e) => SearchFilter(e)}
            mt={2}
            width={iSmallScreen ? "60%" : "30%"}
            border="1px"
            borderColor="gray.500"
            height="80%"
            isDisabled={!showDatabaseData}
            placeholder={
              selectedTable === "Registros" ? "Orden de Compra"
