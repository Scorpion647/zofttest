...
import {
  ...
  selectSingleBill,
  updateBills,
} from "@/app/_lib/database/base_bills";
import {
  deleteMaterial,
  insertMaterial,
  selectMaterials,
  ...
  updateMaterial,
} from "@/app/_lib/database/materials";
import {
  insertSupplier,
  selectSingleSupplier,
  selectSuppliers,
  updateSupplier,
} from "@/app/_lib/database/suppliers";
...
export const Modals = ({ isOpen2, onOpen2, onClose2, Case }) => {
  ...
  const fetchMaterialData = async () => {
    if (Case === "Materiales") {
      ...
      try {
        const data = await selectBills({
          page: 1,
          limit: 1,
          equals: { purchase_order: formDataa.input4, item: formDataa.input5 },
        }); // Implementa esta función para traer los datos
      ...
      try {
        const data = await selectSuppliers({
          page: 1,
          limit: 1,
          equals: { name: inputValue },
        });
  ...
  const handleSubmitt = async () => {
    if (Case === "Materiales") {
      ...
      if (
        ...
        !inputValue ||
        !formDataa.select1 ||
        isNaN(Number(formDataa.input8)) ||
        isNaN(Number(formDataa.input10)) ||
        isNaN(Number(formDataa.input11))
      ) {
        return toast({
          title: "Error",
      ...
      const supplier = await selectSuppliers({
        page: 1,
        limit: 1,
        equals: { name: inputValue },
      });
      ...
      try {
        const response = await updateBills({
          base_bill_id: bill[0]?.base_bill_id,
          material_code: formDataa.input6,
          description: formDataa.input7,
          total_quantity: formDataa.input8,
          measurement_unit: formDataa.input9,
          unit_price: formDataa.input10 * 100,
          net_price: (formDataa.input11 * 100).toFixed(0),
          currency: formDataa.select1,
          supplier_id: sup_id,
        }); // Implementa esta función
        if (!response) {
          toast({
      ...
      if (
        !/^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(formDataa.inputOpcional) &&
        formDataa.inputOpcional
      ) {
        return toast({
          title: "Error",
      ...
      try {
        const response = await updateSupplier({
          ...
          domain: formDataa.inputOpcional,
        });
        if (!response) {
          toast({
            title: "Éxito",
        ...
        {Case === "Proveedores" && (
          <HStack
            ...
            <Tooltip
              textColor="black"
              bgColor="white"
              label="La actualización de un proveedor desactivará la vinculación de todas las Órdenes de Compra y los usuarios asociados actualmente."
              placement="top">
              <Text
                cursor="pointer"
